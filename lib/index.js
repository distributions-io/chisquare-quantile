...
function quantile( p, options ) {
	...
	if ( isTypedArrayLike( p ) ) {
		...
		return quantile6( out, p, opts.k );
	}
	if ( isArrayLike( p ) ) {
		// Handle deepset first...
		if ( opts.path ) {
			opts.sep = opts.sep || '.';
			return quantile4( p, opts.k, opts.path, opts.sep );
		}
		...
		if ( opts.accessor ) {
			return quantile3( out, p, opts.k, opts.accessor );
		}
		return quantile2( out, p, opts.k );
	}
	return NaN;
