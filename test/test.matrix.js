...
describe( 'matrix quantile', function tests() {
	...
	it( 'should throw an error if provided unequal length matrices', function test() {
		...
		function badValues() {
			quantile( matrix( [10,10] ), mat, k );
		}
	});

	it( 'should evaluate the quantile function of the Chi-squared distribution for each matrix element', function test() {
		var actual, i;

		actual = matrix( [5,5], 'float64' );
		actual = quantile( actual, mat, k );


		for ( i = 0; i < actual.length; i++ ) {
			if ( isFiniteNumber( actual.data[ i ] ) && isFiniteNumber( out.data[ i ] ) ) {
				assert.closeTo( actual.data[ i ], out.data[ i ], 1e-10 );
			}
		}
	});
	...
	it( 'should return an empty matrix if provided an empty matrix', function test() {
		...
		mat = matrix( [0,10] );
		assert.deepEqual( quantile( out, mat, k ).data, expected );
