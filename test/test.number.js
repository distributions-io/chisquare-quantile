...
describe( 'number quantile', function tests() {
	...
	it( 'should evaluate the quantile function of the Chi-squared distribution', function test() {
		var actual;
	...
	it( 'should return `NaN` if provided `NaN` as input', function test() {
		assert.isTrue( isnan( quantile( NaN, k ) ) );
	});

	it( 'should return `NaN` if provided a number outside [0,1]', function test() {
		assert.isTrue( isnan( quantile( 1.1, k ) ) );
		assert.isTrue( isnan( quantile( -0.1, k ) ) );
	});
